...
export default function Send({ token }) {
  ...
  const { lastEvent } = useWebSocket(user.wallet);

  // If a validator cancels our pending tx via quorum, detect it here
  useEffect(() => {
    if (lastEvent && pendingTx && lastEvent.txId === pendingTx.txId && lastEvent.status === 'cancelled') {
      setPendingTx(null);
      setError(lastEvent.reason || t('cancelled_by_validators') || 'Transaction cancelled by network validators');
    }
  }, [lastEvent]);
