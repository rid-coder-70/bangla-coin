...
import { ArrowUpRight, ArrowDownLeft, RefreshCw, Send, CheckCircle2, Clock, CheckCircle, Activity, LayoutGrid, XCircle, Building2, Wifi, WifiOff } from 'lucide-react';
...
const walletCache = {};
  ...
  // WebSocket: auto-refresh when a txUpdate event arrives
  const { lastEvent, connected } = useWebSocket(user.wallet);
        ...
        <div className="relative z-10">
          ...
          {loading
            ...
            : (() => {
                ...
                const intPart = Math.floor(bal).toLocaleString();
                const decPart = bal.toFixed(3).split('.')[1];
                return (
                  <p className="text-6xl font-black tracking-tight mb-5">
                    {intPart}<span className="text-2xl text-white/50">.{decPart}</span>
