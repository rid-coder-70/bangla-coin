...
router.post('/create', authenticateToken, (req, res) => {
  ...
  const generatePhone = () => {
    let p;
    do { p = '888' + Math.floor(10000000 + Math.random() * 90000000).toString(); } 
    while (db.prepare('SELECT 1 FROM dao_groups WHERE phone = ?').get(p));
    return p;
  };
  const phone = generatePhone();

  const info = db.prepare('INSERT INTO dao_groups (name, owner, contract, phone) VALUES (?, ?, ?, ?)')
    .run(name.trim(), owner, process.env.DAO_CONTRACT || '', phone);
  const groupId = info.lastInsertRowid;
  db.prepare('INSERT OR IGNORE INTO dao_members (group_id, address) VALUES (?, ?)').run(groupId, owner);
  res.json({ message: 'Community created', groupId, communityId: phone });
});

// DELETE /dao/community/:groupId — Delete community (only if 2 or fewer members)
router.delete('/community/:groupId', authenticateToken, (req, res) => {
  const gid = req.params.groupId;
  if (!isMember(gid, req.user.wallet)) return res.status(403).json({ error: 'Not a member' });

  const total = memberCount(gid);
  if (total > 2) return res.status(400).json({ error: 'Cannot delete community with more than 2 members' });

  db.transaction(() => {
    db.prepare('DELETE FROM dao_join_votes WHERE request_id IN (SELECT id FROM dao_join_requests WHERE group_id = ?)').run(gid);
    db.prepare('DELETE FROM dao_join_requests WHERE group_id = ?').run(gid);
    db.prepare('DELETE FROM dao_remove_votes WHERE poll_id IN (SELECT id FROM dao_remove_polls WHERE group_id = ?)').run(gid);
    db.prepare('DELETE FROM dao_remove_polls WHERE group_id = ?').run(gid);
    db.prepare('DELETE FROM dao_votes WHERE proposal_id IN (SELECT id FROM dao_proposals WHERE group_id = ?)').run(gid);
    db.prepare('DELETE FROM dao_proposals WHERE group_id = ?').run(gid);
    db.prepare('DELETE FROM dao_members WHERE group_id = ?').run(gid);
    db.prepare('DELETE FROM dao_groups WHERE id = ?').run(gid);
  })();
...
router.get('/my-groups', authenticateToken, (req, res) => {
  const groups = db.prepare(`
    SELECT g.id, g.name, g.phone, (SELECT COUNT(*) FROM dao_members WHERE group_id = g.id) as member_count
    FROM dao_groups g
    JOIN dao_members m ON m.group_id = g.id
    WHERE m.address = ?
...
// GET /dao/info/:groupId — Community details
router.get('/info/:groupId', authenticateToken, (req, res) => {
  const group = db.prepare(`
    SELECT g.id, g.name, g.owner, g.phone, (SELECT COUNT(*) FROM dao_members WHERE group_id = g.id) as member_count
    FROM dao_groups g
    WHERE g.id = ?
...
// POST /dao/join — Request to join a community by phone number
router.post('/join', authenticateToken, (req, res) => {
  const { groupId: phoneStr } = req.body;
  const p = String(phoneStr).replace('#', '').trim();
  if (!p) return res.status(400).json({ error: 'Community number required' });

  const group = db.prepare('SELECT id, phone FROM dao_groups WHERE phone = ? OR id = ?').get(p, Number(p));
  if (!group) return res.status(404).json({ error: 'Community not found. Check the number.' });
  const gid = group.id;
...
router.post('/propose', authenticateToken, async (req, res) => {
  ...
  // Resolve phone to wallet
  const target = db.prepare('SELECT wallet_address FROM users WHERE phone = ?').get(recipient);
  if (!target) return res.status(404).json({ error: `No user found with phone ${recipient}` });
  const actualRecipient = target.wallet_address;
  ...
  const propInfo = db.prepare('INSERT INTO dao_proposals (group_id, proposer, recipient, amount, description, total_members) VALUES (?,?,?,?,?,?)')
    .run(groupId, req.user.wallet, actualRecipient, amount, description || '', totalMembers);
...
router.post('/vote', authenticateToken, async (req, res) => {
  ...
  if (updated.yes_votes * 2 > updated.total_members) {
    ...
    notifyTxUpdate(updated.recipient, {
      txId: info.lastInsertRowid,
      status: 'executed',
      amount: updated.amount,
      sender: process.env.DAO_CONTRACT || 'DAO Treasury'
    });
  }
...
router.get('/treasury/:groupId', authenticateToken, (req, res) => {
  const group = db.prepare('SELECT id, phone FROM dao_groups WHERE id = ?').get(req.params.groupId);
  if (!group || !group.phone) return res.json({ treasury: 0 }); // handle misconfigured legacy groups
  
  const recipientStr = `DAO_${group.id}_${group.phone}`;
  
  const incoming = db.prepare(
    "SELECT COALESCE(SUM(amount),0) as total FROM transactions WHERE recipient = ? AND status = 'executed'"
  ).get(recipientStr).total;
  
  const spent = db.prepare(
    "SELECT COALESCE(SUM(amount),0) as total FROM dao_proposals WHERE group_id = ? AND executed = 1"
  ).get(req.params.groupId).total;
  
  res.json({ treasury: Math.max(0, incoming - spent) });
});
