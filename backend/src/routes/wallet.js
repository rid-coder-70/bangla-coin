// backend/src/routes/wallet.js — real balance from initial_credit + transactions
const express = require('express');
const db      = require('../db');
const { authenticateToken } = require('./auth');
const router  = express.Router();

// Real off-chain balance: initial_credit + received_executed - sent_executed
function getOffChainBalance(walletAddr) {
  ...
  return Math.max(0, credit + received - sent);
}
