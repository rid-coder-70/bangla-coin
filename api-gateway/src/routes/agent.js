// api-gateway/src/routes/agent.js — Agent Portal: mint, cash-in, location, nearby agents, chat
...
router.post('/cash-in', authenticateToken, requireAgent, (req, res) => {
  ...
  res.json({ message: `Cash-in successful: ${amt.toLocaleString()} BDT → ${target.name || phone}`, txId: info.lastInsertRowid, recipientName: target.name });
});
...
router.post('/location', authenticateToken, requireAgent, (req, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
    ON CONFLICT(user_id) DO UPDATE SET latitude=excluded.latitude, longitude=excluded.longitude, city=excluded.city, division=excluded.division, updated_at=excluded.updated_at
...
router.get('/nearby', authenticateToken, requireAgent, (req, res) => {
  ...
  let agents;
  if (range === 'big') {
    agents = db.prepare(`SELECT u.id, u.name, u.phone, al.city, al.division FROM agent_locations al JOIN users u ON u.id = al.user_id WHERE u.role = 'agent' AND al.division = ? AND u.id != ? ORDER BY al.updated_at DESC`).all(myLoc.division, req.user.id);
  } else if (range === 'medium') {
    agents = db.prepare(`SELECT u.id, u.name, u.phone, al.city, al.division FROM agent_locations al JOIN users u ON u.id = al.user_id WHERE u.role = 'agent' AND al.division = ? AND u.id != ? ORDER BY al.city = ? DESC, al.updated_at DESC`).all(myLoc.division, req.user.id, myLoc.city);
  } else {
    agents = db.prepare(`SELECT u.id, u.name, u.phone, al.city, al.division FROM agent_locations al JOIN users u ON u.id = al.user_id WHERE u.role = 'agent' AND al.city = ? AND al.division = ? AND u.id != ? ORDER BY al.updated_at DESC`).all(myLoc.city, myLoc.division, req.user.id);
  }
...
router.post('/chat/send', authenticateToken, requireAgent, (req, res) => {
  ...
  db.prepare(`INSERT INTO agent_chat_messages (sender_id, sender_name, message, range, city, division) VALUES (?, ?, ?, ?, ?, ?)`).run(req.user.id, user?.name || 'Agent', message.trim(), validRange, myLoc.city, myLoc.division);
...
router.get('/chat/messages', authenticateToken, requireAgent, (req, res) => {
  ...
  let messages;
  if (range === 'big') {
    messages = db.prepare(`SELECT * FROM agent_chat_messages WHERE division = ? AND (range = 'near' OR range = 'medium' OR range = 'big') ORDER BY created_at DESC LIMIT 100`).all(myLoc.division);
  } else if (range === 'medium') {
    messages = db.prepare(`SELECT * FROM agent_chat_messages WHERE division = ? AND (range = 'near' OR range = 'medium') ORDER BY created_at DESC LIMIT 100`).all(myLoc.division);
  } else {
    messages = db.prepare(`SELECT * FROM agent_chat_messages WHERE city = ? AND division = ? AND range = 'near' ORDER BY created_at DESC LIMIT 100`).all(myLoc.city, myLoc.division);
  }
