...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, (err, decodedUser) => {
    ...
    const dbUser = db.prepare('SELECT id, phone, wallet_address, role FROM users WHERE id = ?').get(decodedUser.id);
    if (!dbUser) return res.status(401).json({ error: 'User not found. Please log out and log in again.' });
    
    req.user = { id: dbUser.id, phone: dbUser.phone, wallet: dbUser.wallet_address, role: dbUser.role };
    next();
  });
}
...
router.post('/verify-otp', (req, res) => {
  ...
  if (!user) {
    ...
    const encryptedKey = wallet.privateKey; // MVP: plain text for now
    const userName = req.body.name || '';
    
    const DEFAULT_CREDIT = 5000; // Every new user gets 5000 BDT
    const info = db.prepare('INSERT INTO users (phone, name, wallet_address, encrypted_key, initial_credit) VALUES (?, ?, ?, ?, ?)')
      .run(phone, userName, wallet.address, encryptedKey, DEFAULT_CREDIT);
